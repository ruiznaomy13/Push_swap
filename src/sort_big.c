...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/07/05 12:27:25 by ncastell          #+#    #+#             */
/*   Updated: 2023/07/13 12:52:36 by ncastell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	push_a(t_stack *a, t_stack *b, int max_index)
{
	while (b->first)
	{
		...
		if (b->size > 1)
			ft_push(b, a, 'a');
		while (max_location(b) != 0)
		{
			if (max_location(b) > max_index / 2)
				ft_rrotate(b, 'b');
			else
				ft_rotate(b, 'b');
		}
...
int	less_moves(t_stack *stack, int pos_max)
{
	...
	moves_max = max_location(stack);
	if (sec_max_loc(stack) > pos_max)
		moves_sec = stack->size - sec_max_loc(stack);
	if (max_location(stack) > pos_max)
		moves_max = stack->size - max_location(stack);
	if (moves_sec > moves_max)
		return (max_location(stack));
